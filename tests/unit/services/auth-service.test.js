...
import { createToken, sendEmail } from '../../../src/utils/helper/index.js';
import logger from '../../../src/utils/logger/index.js';
import { createMockUser } from '../../helpers/unit-helper.js';
...
describe('Auth Service Unit Tests', () => {
  ...
  describe('register()', () => {
    ...
    it('should throw 409 if username or email is already taken', async () => {
      const mongoError = new Error('Duplicate Key');
      mongoError.code = 11000;

      vi.mocked(User.create).mockRejectedValue(mongoError);
    ...
    it('should create new user and return safe payload', async () => {
      const mockUser = createMockUser({
        ...input,
        toObject: vi.fn().mockReturnValue({ ...input }),
      });
  ...
  describe('initiatePasswordReset()', () => {
    ...
    it('should do nothing if user not found', async () => {
      vi.mocked(User.findOne).mockReturnValue(createSelectMock(null));
    ...
    it('should generate token, save user and send email', async () => {
      ...
      vi.mocked(User.findOne).mockReturnValue(createSelectMock(mockUser));
