import { vi, describe, beforeEach, it, expect } from 'vitest';
import commentService from '../../../src/modules/comment/comment-service.js';
import Post from '../../../src/db/models/Post.js';
import Comment from '../../../src/db/models/Comment.js';
import AppError from '../../../src/utils/error/AppError.js';
import { POST_STATUSES } from '../../../src/constants/post.js';
import {
  genObjectId,
  createMockUser,
  createMockPost,
  createMockComment,
} from '../../helpers/unit-helper.js';

vi.mock('../../../src/db/models/Post.js', () => ({
  default: {
    findById: vi.fn(),
  },
}));

vi.mock('../../../src/db/models/Comment.js', () => ({
  default: {
    find: vi.fn(),
    create: vi.fn(),
    findOneAndUpdate: vi.fn(),
    exists: vi.fn(),
    findById: vi.fn(),
    deleteOne: vi.fn(),
  },
}));

// Mock Mongoose chain: .select()
const createSelectMock = (data) => ({
  select: vi.fn().mockResolvedValue(data),
});

// Mock Mongoose chain: .sort().limit().populate().lean()
const createSortMock = (data) => ({
  sort: vi.fn().mockReturnThis(),
  limit: vi.fn().mockReturnThis(),
  populate: vi.fn().mockReturnThis(),
  lean: vi.fn().mockResolvedValue(data),
});

// Mock Mongoose chain: .populate().lean()
const createPopulateMock = (data) => ({
  populate: vi.fn().mockReturnThis(),
  lean: vi.fn().mockResolvedValue(data),
});

describe('Comment Service Unit Tests', () => {
  let mockId, mockUser;
  beforeEach(() => {
    vi.clearAllMocks();
    mockId = genObjectId();
    mockUser = createMockUser();
  });

  describe('listLazy()', () => {
    it('should throw 404 if post not found', async () => {
      vi.mocked(Post.findById).mockReturnValue(createSelectMock(null));

      await expect(commentService.listLazy(mockId, null, {})).rejects.toThrow(AppError);
      await expect(commentService.listLazy(mockId, null, {})).rejects.toMatchObject({
        message: 'Post not found',
        statusCode: 404,
        isOperational: true,
      });
    });

    it('should throw 404 if user tries to access drafts of other users', async () => {
      const mockPost = createMockPost({
        _id: mockId,
        status: POST_STATUSES.DRAFT,
      });

      vi.mocked(Post.findById).mockReturnValue(createSelectMock(mockPost));

      await expect(commentService.listLazy(mockId, mockUser, {})).rejects.toThrow(AppError);
      await expect(commentService.listLazy(mockId, mockUser, {})).rejects.toMatchObject({
        message: 'Post not found',
        statusCode: 404,
        isOperational: true,
      });
    });

    it('should return empty array and meta data when no comments found', async () => {
      const mockPost = createMockPost({
        _id: mockId,
        author: mockUser._id,
        status: POST_STATUSES.DRAFT,
      });

      vi.mocked(Post.findById).mockReturnValue(createSelectMock(mockPost));
      vi.mocked(Comment.find).mockReturnValue(createSortMock([]));

      const result = await commentService.listLazy(mockId, mockUser, {});

      expect(result.message).toBe('No comments available');
      expect(result.comments).toEqual([]);
      expect(result.meta.nextCursor).toBeNull();
      expect(result.meta.hasMore).toBe(false);
    });

    it('should handle lazy loading correctly', async () => {
      const mockPost = createMockPost({
        _id: mockId,
        status: POST_STATUSES.PUBLISHED,
      });

      const mockComments = [createMockComment(), createMockComment()];

      vi.mocked(Post.findById).mockReturnValue(createSelectMock(mockPost));
      vi.mocked(Comment.find).mockReturnValue(createSortMock(mockComments));

      const result = await commentService.listLazy(mockId, mockUser, {
        cursor: null,
        limit: 2,
        sortOrder: 'desc',
      });

      expect(result.message).toBe('Comments fetched successfully');
      expect(result.comments).toHaveLength(2);
      expect(result.meta.nextCursor.toString()).toBe(mockComments[1]._id.toString());
      expect(result.meta.hasMore).toBe(false);
    });

    it('should fetch next page of results when cursor is provided', async () => {
      const mockPost = createMockPost({
        _id: mockId,
        status: POST_STATUSES.PUBLISHED,
      });

      const existingCursor = genObjectId();
      const mockComments = [createMockComment(), createMockComment(), createMockComment()];

      vi.mocked(Post.findById).mockReturnValue(createSelectMock(mockPost));
      vi.mocked(Comment.find).mockReturnValue(createSortMock(mockComments));

      const result = await commentService.listLazy(mockId, mockUser, {
        cursor: existingCursor.toString(),
        limit: 2,
        sortOrder: 'desc',
      });

      expect(Comment.find).toHaveBeenCalledWith({
        _id: { $lt: existingCursor.toString() },
        post: mockPost._id,
      });

      expect(result.message).toBe('Comments fetched successfully');
      expect(result.comments).toHaveLength(2);
      expect(result.meta.nextCursor.toString()).toBe(mockComments[1]._id.toString());
      expect(result.meta.hasMore).toBe(true);
    });
  });

  describe('add()', () => {
    const input = { content: 'hi' };

    it('should create new comment and return populated comment payload for PUBLISHED post', async () => {
      const mockPost = createMockPost({
        _id: mockId,
        status: POST_STATUSES.PUBLISHED,
      });

      const mockCommentObj = createMockComment({
        content: input.content,
        user: mockUser._id,
        post: mockPost._id,
      });

      const mockCommentDoc = {
        ...mockCommentObj,
        populate: vi.fn().mockReturnThis(),
        toObject: vi.fn().mockReturnValue(mockCommentObj),
      };

      vi.mocked(Post.findById).mockReturnValue(createSelectMock(mockPost));
      vi.mocked(Comment.create).mockResolvedValue(mockCommentDoc);

      const result = await commentService.add(mockId, mockUser, input);

      expect(Comment.create).toHaveBeenCalledWith({
        content: 'hi',
        post: mockPost._id,
        user: mockUser._id,
      });
      expect(mockCommentDoc.populate).toHaveBeenCalledWith('user', 'username');
      expect(mockCommentDoc.toObject).toHaveBeenCalledOnce();
      expect(result.content).toBe('hi');
    });

    it('should create new comment and return populated comment payload for DRAFT post if user is post author', async () => {
      const mockPost = createMockPost({
        author: mockUser._id,
        status: POST_STATUSES.DRAFT,
      });

      const mockCommentObj = createMockComment({
        content: input.content,
        user: mockUser._id,
        post: mockPost._id,
      });

      const mockCommentDoc = {
        ...mockCommentObj,
        populate: vi.fn().mockReturnThis(),
        toObject: vi.fn().mockReturnValue(mockCommentObj),
      };

      vi.mocked(Post.findById).mockReturnValue(createSelectMock(mockPost));
      vi.mocked(Comment.create).mockResolvedValue(mockCommentDoc);

      const result = await commentService.add(mockId, mockUser, input);

      expect(Comment.create).toHaveBeenCalledWith({
        content: 'hi',
        post: mockPost._id,
        user: mockUser._id,
      });
      expect(mockCommentDoc.populate).toHaveBeenCalledWith('user', 'username');
      expect(mockCommentDoc.toObject).toHaveBeenCalledOnce();
      expect(result.content).toBe('hi');
    });
  });

  describe('edit()', () => {
    const input = { content: 'new' };
    it('should throw 404 if could not find both original and updated comment', async () => {
      vi.mocked(Comment.findOneAndUpdate).mockReturnValue(createPopulateMock(null));
      vi.mocked(Comment.exists).mockResolvedValue(null);

      await expect(commentService.edit(mockId, mockUser, input)).rejects.toThrow(AppError);
      await expect(commentService.edit(mockId, mockUser, input)).rejects.toMatchObject({
        message: 'Comment not found',
        statusCode: 404,
        isOperational: true,
      });
    });

    it('should throw 403 if original comment exists and updated comment not found', async () => {
      vi.mocked(Comment.findOneAndUpdate).mockReturnValue(createPopulateMock(null));
      vi.mocked(Comment.exists).mockResolvedValue({ _id: mockId });

      await expect(commentService.edit(mockId, mockUser, input)).rejects.toThrow(AppError);
      await expect(commentService.edit(mockId, mockUser, input)).rejects.toMatchObject({
        message: 'You are not authorized to edit this comment',
        statusCode: 403,
        isOperational: true,
      });
    });

    it('should return updated comment if updated comment found', async () => {
      const mockComment = createMockComment({
        _id: mockId,
        content: input.content,
        user: mockUser,
      });

      vi.mocked(Comment.findOneAndUpdate).mockReturnValue(createPopulateMock(mockComment));

      const result = await commentService.edit(mockId, mockUser, input);

      expect(result.content).toBe('new');
    });
  });

  describe('remove()', () => {
    it('should throw 404 if comment not found', async () => {
      vi.mocked(Comment.findById).mockReturnValue(createSelectMock(null));

      await expect(commentService.remove(mockId, mockUser)).rejects.toThrow(AppError);
      await expect(commentService.remove(mockId, mockUser)).rejects.toMatchObject({
        message: 'Comment not found',
        statusCode: 404,
        isOperational: true,
      });
    });

    it('should throw 403 if user is not post author or comment owner', async () => {
      const mockPost = createMockPost();
      const mockComment = createMockComment({
        _id: mockId,
        user: genObjectId(),
        post: mockPost._id,
      });

      vi.mocked(Comment.findById).mockReturnValue(createSelectMock(mockComment));
      vi.mocked(Post.findById).mockReturnValue(createSelectMock(mockPost));

      await expect(commentService.remove(mockId, mockUser)).rejects.toThrow(AppError);
      await expect(commentService.remove(mockId, mockUser)).rejects.toMatchObject({
        message: 'You are not authorized to delete this comment',
        statusCode: 403,
        isOperational: true,
      });
    });

    it('should delete comment if user is comment owner', async () => {
      const mockPost = createMockPost();
      const mockComment = createMockComment({
        _id: mockId,
        user: mockUser._id,
        post: mockPost._id,
      });

      vi.mocked(Comment.findById).mockReturnValue(createSelectMock(mockComment));
      vi.mocked(Post.findById).mockReturnValue(createSelectMock(mockPost));

      await commentService.remove(mockId, mockUser);

      expect(Comment.deleteOne).toHaveBeenCalledWith({ _id: mockComment._id });
    });

    it('should delete comment if user is post author', async () => {
      const mockPost = createMockPost({
        author: mockUser._id,
      });
      const mockComment = createMockComment({
        _id: mockId,
        user: genObjectId(),
        post: mockPost._id,
      });

      vi.mocked(Comment.findById).mockReturnValue(createSelectMock(mockComment));
      vi.mocked(Post.findById).mockReturnValue(createSelectMock(mockPost));

      await commentService.remove(mockId, mockUser);

      expect(Comment.deleteOne).toHaveBeenCalledWith({ _id: mockComment._id });
    });
  });
});
